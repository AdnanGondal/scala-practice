...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    ...
    // apply(index)
    grades.sortBy(_.ordinal).apply(if (size%2 == 0) size/2-1 else size/2)
...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def elect(ballots: Seq[Ballot]): Candidate =
    ...
    // each ballot: grade assigned to each candidate:
    ...
    allGrades.groupMap((c, g) => c)(_._2)

    //group by candiate,   // transforming over the keys
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      gradesPerCandidate.filter((c,g)=> Grade.median(g) == bestMedianGrade)
      ...
      if bestCandidates.size == 1 then
        ...
        bestCandidates.map((c,g)=> (c,g.diff(List(bestMedianGrade))))
