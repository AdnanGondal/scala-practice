...
object SimulatePlanApp extends App {
  ...
  // assuming args has at least 6 elems
  def parseParams(args: Array[String]): RetCalcResult[RetCalcParams] = {
    (parseInt("nbOfYearsRetired", args(2)),
      ...
      parseInt("initialCapital", args(5))) // accumulate errors:
      .mapN{ case (nbOfYearsRetired, netIncome, currentExpenses, initialCapital)=>{
        RetCalcParams(nbOfMonthsInRetirement = nbOfYearsRetired * 12,
          netIncome = netIncome,
          currentExpenses = currentExpenses,initialCapital=initialCapital)
      }}
  }

  // function that calls RetCalc simulatePlan
  def strSimulatePlan(returns: Returns, nbOfYearsSaving: Int, params: RetCalcParams): RetCalcResult[String] = {
    RetCalc.simulatePlan(
      ...
      case (capitalAtRetirement, capitalAfterDeath) =>
        ...
        s"""
           ...
           |""".stripMargin
    }.toValidatedNel  // converts Either to ValidatedNel
  }
